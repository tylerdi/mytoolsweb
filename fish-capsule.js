...
(function () {
  ...
  class FishCapsule {
    ...
    render() {
      ...
      if (opened.length > 0) this.saveCapsules();
      ...
      this.container.innerHTML = `
        <style>
          ...
          .cap-textarea {
            width: 100%; background: #0a0a0a; border: 1px solid var(--border, #2a2a2a);
            border-radius: 10px; padding: 12px; color: var(--text, #e8e8e8);
            font-size: 0.9rem; font-family: inherit; resize: none;
            min-height: 80px; outline: none; line-height: 1.7;
          }
          ...
          .cap-date {
            flex: 1; background: #0a0a0a; border: 1px solid var(--border, #2a2a2a);
          ...
          .cap-letter {
            display: none; padding: 16px; background: #0a0a0a;
        ...
        <div class="cap-widget">
          ...
          <div class="cap-form">
            ...
            <div class="cap-row">
              ...
              <button class="cap-send" onclick="this.closest('.cap-widget').__send()">💌 封存</button>
            </div>
          </div>
          ${pending.length > 0 ? `
            <div class="cap-list">
              ...
              ${pending.map((c, i) => `
                <div class="cap-item" onclick="this.closest('.cap-widget').__toggle(${c.id})">
                  ...
                  <div class="preview">${c.text.slice(0, 30)}...</div>
      ...
      for (const c of opened) {
        ...
        toast.style.cssText = 'position:fixed;top:80px;left:50%;transform:translateX(-50%);background:linear-gradient(135deg,#d4a853,#f0d78c);color:#000;padding:12px 24px;border-radius:12px;font-weight:700;font-size:0.9rem;z-index:9999;box-shadow:0 8px 30px rgba(212,168,83,0.4);animation:fcToastIn 0.4s ease;';
